...
public class MazeGenerator : MonoBehaviour {
	...
	public void MakeExit () {
		...
		print("Exit position: " + newCell.cellObject.transform.position);
		GameObject exit = Instantiate(entrancePrefab);
		// Remove appropriate wall for chosen edge cell.
		if (newCell.gridPos.x == 0) { //Left
			RemoveWall(newCell.cScript, 1);
			exit.transform.position = newCell.cellObject.transform.position + Vector3.left * cellSize;
			exit.transform.eulerAngles = new Vector3(90f, 90f, 0f);
		} else if (newCell.gridPos.x == mazeColumns) {
			RemoveWall(newCell.cScript, 2); //Right
			exit.transform.position = newCell.cellObject.transform.position + Vector3.right * cellSize;
			exit.transform.eulerAngles = new Vector3(90f, 270f, 0f);
		} else if (newCell.gridPos.y == mazeRows) {
			RemoveWall(newCell.cScript, 3); //Up
			exit.transform.position = newCell.cellObject.transform.position + Vector3.forward * cellSize;
			exit.transform.eulerAngles = new Vector3(90f, 180f, 0f);
		} else {
			RemoveWall(newCell.cScript, 4); //Down
			exit.transform.position = newCell.cellObject.transform.position + Vector3.back * cellSize;
			exit.transform.eulerAngles = new Vector3(90f, 0f, 0f);
		}
	...
	public void RemoveWall (MazeCell cScript, int wallID) {
	...
	public void GenerateCell (Vector2 pos, Vector2 keyPos) {
		...
		newCell.cellObject = Instantiate(cellPrefab, new Vector3(pos.x, 0f, pos.y), cellPrefab.transform.rotation);
		...
		newCell.cScript = newCell.cellObject.GetComponent<MazeCell>();
...
//	public bool centerMaze = true;
//	public float cellWidth = 2f;
//	public int mazeColumns = 20;
//	private int mazeWidth;
//	public int mazeRows = 20;
//	private int mazeHeight;
//	public float secondaryPathChance = 0.4f;
...
//	void GenerateMaze () {
//		if (!wall) {
//			print("Please specify a non-null wall prefab to generate a maze with.");
//			return;
//		}
//		mazeWidth = mazeColumns * 2 + 1;
//		mazeHeight = mazeRows * 2 + 1;

//		GameObject[,] maze = new GameObject[mazeWidth, mazeHeight];
//		bool[,] visited = new bool[mazeColumns, mazeRows];
//		bool[,] opening = new bool[mazeWidth, mazeHeight];

//		for (int j = 0; j < mazeHeight; j++) {
//			for (int i = 0; i < mazeWidth; i++) {
//				if (i % 2 == 1 && j % 2 == 1) {
//					visited[(i-1)/2, (j-1)/2] = false;
//					opening[(i-1)/2, (j-1)/2] = false;
//				}
//				Vector3 pos = transform.position + new Vector3(i, 0, j) * cellWidth;
//				if (centerMaze) pos -= new Vector3(mazeWidth * cellWidth / 2f, 0f, mazeHeight * cellWidth / 2f);
//				maze[i, j] = Instantiate(wall, pos, Quaternion.identity);
//			}
//		}
//		int visitedCount = 0;
//		int x = 0;
//		int y = 0;
//		while (visitedCount < mazeColumns * mazeRows) {
//			visited[x, y]
//		}
...
//				//	for (int i = 0; i < mazeWidth; i++) {
//				//		visited[i, j] = false;
//				//		//maze[i, j] = Instantiate(wall, transform.position + new Vector3(i, j) * cellWidth, Quaternion.identity);
